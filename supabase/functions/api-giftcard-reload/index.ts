// MySargal — api-giftcard-reload · x-api-key · rate-limit · webhook giftcard.reloaded
...
serve(async (req) => {
  ...
  try {
    ...
    const { code, amount, reference } = await req.json().catch(() => ({}))
    if (!code || !amount) return bad('code et amount requis')
    const amt = Math.round(Number(amount))
    if (!Number.isFinite(amt) || amt <= 0) return bad('amount invalide')
    const { data, error } = await sb.rpc('gift_card_partner_reload', { p_merchant: merchant, p_code: String(code).toUpperCase().trim(), p_amount: amt, p_reference: reference ?? null })
    if (error) return bad(error.message, 500)
    const r = data as any
    if (!r?.success) return ok(r, r?.error === 'not_found_or_inactive' ? 404 : 400)
    await dispatch(sb, partner, 'giftcard.reloaded', { code: String(code).toUpperCase().trim(), amount_added: r.amount_added, balance_remaining: r.balance_remaining, reference: reference ?? null })
    return ok(r)
