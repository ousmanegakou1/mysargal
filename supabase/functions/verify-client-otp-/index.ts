// ============================================================
// MySargal — verify-client-otp : vérifie le code WhatsApp du client et émet
// un JWT MySargal (claim phone) — accepté par enroll-loyalty.
// Body : { phone, code }  →  { token }
// Déploiement : supabase functions deploy verify-client-otp --no-verify-jwt
// Secrets : MS_JWT_SECRET, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
// ============================================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Dernier code non utilisé pour ce numéro
    const { data: rows } = await sb.from('client_otps')
    ...
    if (hash !== otp.code_hash) {
      await sb.from('client_otps').update({ attempts: otp.attempts + 1 }).eq('id', otp.id)
      return json({ error: 'Code incorrect' }, 401)
    }

    // OK → consommer le code
    await sb.from('client_otps').update({ used: true }).eq('id', otp.id)

    // Émettre le JWT MySargal (30 jours), même format que verify-whatsapp-otp
