// MySargal — api-giftcard-void · x-api-key · rate-limit · webhook giftcard.voided
...
serve(async (req) => {
  ...
  try {
    ...
    if (!authorization_code) return bad('authorization_code requis')
    const { data, error } = await sb.rpc('gift_card_partner_void', { p_merchant: merchant, p_auth: String(authorization_code).trim() })
    if (error) return bad(error.message, 500)
    const r = data as any
    if (!r?.success) return ok(r, r?.error === 'auth_not_found' ? 404 : 400)
    await dispatch(sb, partner, 'giftcard.voided', { authorization_code: String(authorization_code).trim(), amount_reversed: r.amount_reversed, balance_remaining: r.balance_remaining })
    return ok(r)
