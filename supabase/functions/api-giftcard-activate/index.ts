// MySargal — api-giftcard-activate · x-api-key · rate-limit · webhook giftcard.activated
...
serve(async (req) => {
  ...
  try {
    ...
    const { amount, recipient_phone, reference } = await req.json().catch(() => ({}))
    if (!amount) return bad('amount requis')
    const amt = Math.round(Number(amount))
    if (!Number.isFinite(amt) || amt <= 0) return bad('amount invalide')
    const { data, error } = await sb.rpc('gift_card_partner_activate', { p_merchant: merchant, p_amount: amt, p_phone: recipient_phone ?? null, p_reference: reference ?? null })
    if (error) return bad(error.message, 500)
    const r = data as any
    if (!r?.success) return ok(r, 400)
    await dispatch(sb, partner, 'giftcard.activated', { code: r.code, balance: r.balance, recipient_phone: recipient_phone ?? null, reference: reference ?? null })
    return ok(r)
